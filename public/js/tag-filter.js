...
(function () {
  ...
  function TagFilter() {
    ...
    this.tagButtons = document.querySelectorAll(
      '[data-tag]:not([data-tag="all"])'
    );
    this.postItems = document.querySelectorAll(".post-item");
    // Try to find results count - check both possible classes
    this.resultsCount =
      document.querySelector(".mb-8 .text-gray-700") ||
  ...
  TagFilter.prototype.filterPosts = function () {
    ...
    this.postItems.forEach(function (post) {
      var postTags = (post.dataset.tags || "")
        .split(",")
        .filter(function (tag) {
          return tag.trim() !== "";
        });
      ...
      if (self.selectedTags.size === 0) {
        ...
        var hasSelectedTag = Array.from(self.selectedTags).some(function (
          selectedTag
        ) {
    ...
    if (this.resultsCount) {
      if (this.selectedTags.size === 0) {
        this.resultsCount.textContent =
          "Showing all posts (" + visibleCount + " total)";
      } else {
